...
(function () {
  ...
  window.searchAlphaFold = async function () {
    ...
    if (!sequence && !fullname && !name) {
      Utils.showToast('Remplissez la séquence ou le nom de la protéine d\'abord', 'warning');
      return;
    }
  ...
  async function populateProteinSelect() {
    ...
    try {
      const allProteins = (await API.getAllProteins()).data || [];
  ...
  window.toggleLabels = function (show) {
    ...
    if (show) {
      ...
      atoms.forEach((atom, i) => {
        ...
        const code3 = (atom.resn || '').toUpperCase();
        const letter = AA3TO1_LBL[code3] || code3;
        viewer.addLabel(`${letter}${atom.resi}`, {
          position: atom,
          fontSize: 9,
          fontColor: 'white',
