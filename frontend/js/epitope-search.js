...
(function () {
  ...
  // ─── Manual epitope entry ─────────────────────────────────────────────────
  window.searchManualEpitope = function () {
    const raw = (document.getElementById('manual-epitope-input').value || '').trim().toUpperCase();

    // Validate: only IUPAC amino-acid letters
    if (!raw) {
      Utils.showToast('Entrez une séquence d\'épitope.', 'warning');
      return;
    }
    if (!/^[ACDEFGHIKLMNPQRSTVWYBXZJUO]+$/.test(raw)) {
      Utils.showToast('Séquence invalide — utilisez uniquement des acides aminés (A–Z).', 'error');
      return;
    }
    if (raw.length < 3) {
      Utils.showToast('L\'épitope doit contenir au moins 3 acides aminés.', 'warning');
      return;
    }
    if (fastaProteins.length === 0) {
    ...
    // Deselect any predicted epitope card
    ...
    for (const fp of fastaProteins) {
      const hits = searchEpitopeInSequence(raw, fp.sequence, minIdentity / 100);
      hits.forEach(h => allHits.push({ protein: fp, ...h }));
    }

    // Build a synthetic epitope object so renderSearchResults works unchanged
    activeEpitope = { sequence: raw, start: '—', end: '—', score: { toFixed: () => 'manuel' } };
  ...
  function renderSearchResults(hits) {
    ...
    const scoreLabel = (ep.score && typeof ep.score.toFixed === 'function') ? ep.score.toFixed(3) : 'manuel';
    const posLabel   = (ep.start === '—') ? 'Saisie manuelle' : `Pos ${ep.start}–${ep.end}`;
    document.getElementById('searched-epitope-meta').textContent =
      `${posLabel} · Score ${scoreLabel} · ${ep.sequence.length} aa`;
