# Rewrites fill_finance.py with correct UTF-8 content (no encoding corruption)
clean = '''\
"""
...
DST  = r\'C:\\Users\\asus\\Downloads\\EpitopX_Analyse_Financiere_v6.xlsx\'
...
infra = [
    (5,  "Appels API LLM (OpenRouter - modeles open-source)", 1000,"abonnements", 14000, 1, "appel/abo"),
    (6,  "Compute cloud variable (Render Starter)",           1000,"abonnements",  4200, 1, "instance"),
    (7,  "Stockage donnees utilisateurs (Cloudflare R2)",     1000,"abonnements",  1400, 1, "GB/abo"),
...
services = [
    (30, "Hebergement Render (Starter - $10/mois)",        1, 1400),
    (31, "API LLM OpenRouter (budget minimal - $10/mois)", 1, 1400),
...
print("OK Charges fixes rempli  [annuel estime: 2 925 360 DA < 3 000 000 DA]")
...
abonnements = [10, 35, 85, 160, 280, 420, 580, 750, 950, 1200, 1500, 1900]
...
charges_fixes = (4 * 60000 + 1400 + 1400 + 980) * 12
var_per_sub  = (14000 + 4200 + 1400 + 73080) / 1000
var_total    = total_subs * var_per_sub
marketing_DA = sum(marketing_monthly) * 1000
equip_total  = 3*60000 + 3*6000 + 3*3000 + 2000 + 15000 + 5000
...
print(f"  Services (Render+LLM+domaine): {(1400+1400+980)*12:>14,.0f} DA")
...
with open('fill_finance.py', 'w', encoding='utf-8') as f:
    f.write(clean)

print("fill_finance.py rewritten successfully")
